...
| Модуль | Назначение |
|--------|------------|
| [`main.py`](./main.py) | CLI-диспетчер проекта. Запуск полного ETL-пайплайна. |
...
| `data`| Хранение сырых CSV/JSON-файлов. |
---
...
### По чемпиону (Используется только для Scatter-графика)
- Выбор из списка
